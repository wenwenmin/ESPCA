...
source('./Program/ESPCA/fun_ESPCA.R')

gse <- data.frame(read_excel("./Program/db/breast_cancer/GSE3494.xlsx"))
gse_data <- as.matrix(gse[,-1]) # remove label

ppi <- data.frame(read_delim("./Program/db/breast_cancer/PPI.txt", "\t", escape_double = FALSE, col_names = FALSE, trim_ws = TRUE))
edges<-ppi
ppi$weight <- 1
